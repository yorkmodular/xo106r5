...
#define CV_INPUT A2
#define TUNE_INPUT A1
#define DUTY_INPUT A3
...
volatile uint16_t syncPhaseAcc, syncPhaseInc, baseFreq, syncOffset, finetune, duty;
...
void audioOn() {
  ...
  // PWM1A - Enable pulsewidth modulator A
  // COM1A0 - Togle OC1A output line on match
...
void loop() {
  ...
  finetune = -128 + (analogRead(TUNE_INPUT) >> 2);
  duty = map(analogRead(DUTY_INPUT), 0, 1023, 16, 240);
